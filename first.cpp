...
class CdCommand{
public:
    void rumCmd(const vector<string>& args){
        ...
        for (const auto& file : files) {
            if (!isHidden(file.path().filename().string()) || all) {
                ...
                if (recursive && fs::is_directory(file.path())) {
                    listDirectory(file.path(), recursive, all, detail, depth + "\t");
                }
            }
        }
        ...
        for (const auto& file : fs::directory_iterator(source)) {
            if (!isHidden(file.path().filename().string())) {
                ...
                if (fs::is_directory(file.path())) {
                    mvDir(file.path(),destinationFilePath,force);
                }else{
    ...
    void rmDir(const fs::path& path, bool recursive, bool interactive){
        for (const auto& file : fs::directory_iterator(path)) {
            if (fs::is_directory(file.path()) && recursive) {
                rmDir(file.path(),recursive,interactive);
            }
            if(!fs::is_directory(file.path())){
                if(interactive){
    ...
    void cpDir(const fs::path& source,const fs::path& destination, bool recursive, bool isPrompt){
        ...
        for (const auto& file : fs::directory_iterator(source)) {
            if (!isHidden(file.path().filename().string())) {
                ...
                if(fs::is_directory(file.path())){
                    if(recursive){
                        cpDir(file.path(),destinationFilePath,recursive,isPrompt);
                    }
                }else{
    ...
    void run(bool isArgCli, char *argv[]) {
        while (true) {
            ...
            if (args.empty()) {
                ...
                cerr << "Unknown command: " << args[0] << ", Please check your command. Only supported commands are cd, ls, cp, mv & rm"<< endl;
            }
            if(isArgCli){
                break;
            }
